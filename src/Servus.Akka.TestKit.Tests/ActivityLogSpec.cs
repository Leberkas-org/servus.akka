...
public sealed class ActivityLogSpec
{
    ...
    public void Record_should_add_entry()
    {
        ...
        var activity = new OutboundReceived(0, TransportData.Rent(new byte[] { 0xAA }.ToWireBuffer()));
    ...
    {
        var log = new ActivityLog();
        var outbound = new OutboundReceived(0, TransportData.Rent(new byte[] { 0xAA }.ToWireBuffer()));
        ...
        var handler = new HandlerInvoked("TestHandler", TransportData.Rent(new byte[] { 0xBB }.ToWireBuffer()));
    ...
    {
        var log = new ActivityLog();
        log.Record(new OutboundReceived(0, TransportData.Rent(new byte[] { 0xAA }.ToWireBuffer())));
        var connectionInfo = new ConnectionInfo(
            new IPEndPoint(IPAddress.Loopback, 1000),
            new IPEndPoint(IPAddress.Loopback, 2000),
            TransportProtocol.Tcp);
